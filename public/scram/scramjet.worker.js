  ...
  var e = {
      8810: function (e, t, r) {
        ...
        let n = self.$scramjet,
          i = Function
        function o() {
          ;(n.codec.encode = i(`return ${n.config.codec.encode}`)()),
            (n.codec.decode = i(`return ${n.config.codec.decode}`)())
        }
        function s(e, t) {
	...
	`),
            ...
            return (function (e, t, r, s) {
              ...
              try {
                ...
                try {
                  i =
                    "string" == typeof e
                      ? a.rewrite_js(e, r.base.href, t || "(unknown)", s)
                      : a.rewrite_js_bytes(e, r.base.href, t || "(unknown)", s)
                } catch (r) {
                  return (
                ...
                let l = performance.now(),
                  { js: d, map: u, scramtag: f, errors: h } = i,
                  g = l - c
                if (
                  ...
                  for (let e of h) console.error("oxc parse error", e)
                ;(0, n.Sp)("rewriterLogs", r.base) &&
                  console.log(
                    `oxc rewrite for "${t || "(unknown)"}" was ${g < 1 ? "BLAZINGLY FAST" : g < 500 ? "decent speed" : "really slow"} (${g}ms)`,
                  )
                return {
                  js: "string" == typeof e ? o.decode(d) : d,
                  tag: f,
                  map: u,
                }
              } finally {
      ...
      522: function (e, t, r) {
        ...
        var n = r(7418),
          i = r(8810)
        async function o() {
          let e = await fetch(i.h3.config.files.wasm).then((e) =>
            e.arrayBuffer(),
          )
        ...
        let s = new TextDecoder(),
          a = "\0asm".split("").map((e) => e.charCodeAt(0))
        function c(e) {
          let t
          if (!(self.REAL_WASM && self.REAL_WASM instanceof Uint8Array))
            throw Error("rewriter wasm not found (was it fetched correctly?)")
          if (![...self.REAL_WASM.slice(0, 4)].every((e, t) => e === a[t]))
            throw Error(
              "rewriter wasm does not have wasm magic (was it fetched correctly?)\nrewriter wasm contents: " +
                s.decode(self.REAL_WASM),
            )
          ;(0, n.rb)({ module: new WebAssembly.Module(self.REAL_WASM) }),
            i.h3.shared.rewriter || (i.h3.shared.rewriter = [])
          let r = i.h3.shared.rewriter.findIndex((e) => !e.inUse),
            o = i.h3.shared.rewriter.length
          return (
            -1 === r
              ? ((0, i.Sp)("rewriterLogs", e.base) &&
                  console.log(
                    `creating new rewriter, ${o} rewriters made already`,
                  ),
                (t = { rewriter: new n.mZ(i.h3), inUse: !0 }),
                i.h3.shared.rewriter.push(t))
              : ((0, i.Sp)("rewriterLogs", e.base) &&
                  console.log(
                    `using cached rewriter ${r} from list of ${o} rewriters`,
                  ),
                (t = i.h3.shared.rewriter[r])),
          ...
          try {
            ...
            if (
              (r.searchParams.has("type") &&
                ((n = r.searchParams.get("type")),
                r.searchParams.delete("type")),
...
self.WASM = '${r}';`),
            ...
            if (
              ...
              let o,
                s = r.pathname.substring(this.config.prefix.length)
              s.startsWith("blob:") && (s = (0, i.Ag)(s))
              let c = await fetch(s, {})
              ;(c.finalURL = s.startsWith("blob:") ? s : "(data url)"),
                c.body &&
                  (o = await d(
                    c,
                    t
                      ...
                      : a(new URL((0, i.Sd)(e.referrer))),
                    e.destination,
                    n,
                    this.cookieStore,
                  ))
              let l = Object.fromEntries(c.headers.entries())
              return (
                crossOriginIsolated &&
            ...
            let s = new URL((0, i.Sd)(r)),
              c = this.serviceWorkers.find((e) => e.origin === s.origin)
            if (
              c &&
              c.connected &&
              "swruntime" !== r.searchParams.get("from")
            ) {
            ...
            let u = new i.KF()
            for (let [t, r] of e.headers.entries()) u.set(t, r)
            if (t && new URL(t.url).pathname.startsWith(o.h3.config.prefix)) {
              let e = new URL((0, i.Sd)(t.url))
              e.toString().includes("youtube.com") ||
                (u.set("Referer", e.toString()),
                u.set("Origin", e.origin ? `${e.protocol}//${e.host}` : "null"))
            }
            let h = this.cookieStore.getCookies(s, !1)
            h.length && u.set("Cookie", h),
              u.set("Sec-Fetch-Dest", e.destination),
              u.set("Sec-Fetch-Site", "same-origin"),
              u.set(
                "Sec-Fetch-Mode",
                "cors" === e.mode ? e.mode : "same-origin",
              )
            let g = new f(s, u.headers, e.body, e.method, e.destination, t)
            this.dispatchEvent(g)
            let w =
              g.response ||
              (await this.client.fetch(g.url, {
                method: g.method,
                body: g.body,
            ...
            return await l(s, n, e.destination, w, this.cookieStore, t, this)
          } catch (o) {
            let t = {
              message: o.message,
              url: e.url,
            ...
            let r = Object.entries(t)
              ...
              .join("\n\n")
            return (0, n.O)(r, (0, i.Sd)(e.url))
          }
        }
        async function l(e, t, r, n, o, s, c) {
          let l,
            f = (0, i.V6)(n.rawHeaders, a(e)),
            h = f["set-cookie"] || []
          for (let t in h)
            if (s) {
              let n = c.dispatch(s, {
              ...
              "document" !== r && "iframe" !== r && (await n)
            }
          for (let t in (await o.setCookies(h instanceof Array ? h : [h], e),
          f))
            Array.isArray(f[t]) && (f[t] = f[t][0])
          if (
            (n.body && (l = await d(n, a(e), r, t, o)),
            ["document", "iframe"].includes(r))
          ) {
            let e = f["content-disposition"]
            if (!/\s*?((inline|attachment);\s*?)filename=/i.test(e)) {
              let t = /^\s*?attachment/i.test(e) ? "attachment" : "inline",
                [r] = new URL(n.finalURL).pathname.split("/").slice(-1)
              f["content-disposition"] = `${t}; filename=${JSON.stringify(r)}`
            }
          }
          "text/event-stream" === f.accept &&
            (f["content-type"] = "text/event-stream"),
            delete f["permissions-policy"],
            crossOriginIsolated &&
              [
                "document",
                "iframe",
          ...
          let g = new u(l, f, n.status, n.statusText, r, e, n, s)
          return (
            c.dispatchEvent(g),
            new Response(g.responseBody, {
              headers: g.responseHeaders,
              status: g.status,
              statusText: g.statusText,
            })
          )
        }
        async function d(e, t, r, n, a) {
          switch (r) {
            case "iframe":
            case "document":
              if (e.headers.get("content-type")?.startsWith("text/html"))
                return (0, i.r4)(await e.text(), a, t, !0)
              return e.body
            case "script": {
              let {
                js: r,
              ...
              if ((0, o.Sp)("sourcemaps", t.base) && a) {
                r instanceof Uint8Array && (r = new TextDecoder().decode(r))
                let e = `${globalThis.$scramjet.config.globals.pushsourcemapfn}([${a.join(",")}], "${i}");`,
                  t = /^\s*(['"])use strict\1;?/
                r = t.test(r)
                  ? r.replace(
                      t,
...
${r}`
      ...
      7418: function (e, t, r) {
        ...
        function l(e, t) {
          return (e >>>= 0), s.decode(c().subarray(e, e + t))
        }
        let d = 0,
          u =
        ...
        function h(e, t, r) {
          ...
          if (s !== n) {
            ...
            let t = f(e, c().subarray(i + s, i + n))
            ;(s += t.written), (i = r(i, n, s, 1) >>> 0)
          }
          return (d = s), i
        }
        let g = null
        function w() {
          return (
            (null === g ||
              !0 === g.buffer.detached ||
              (void 0 === g.buffer.detached && g.buffer !== n.memory.buffer)) &&
              (g = new DataView(n.memory.buffer)),
            g
          )
        }
        function p(e) {
        ...
        class m {
          ...
          rewrite_js(e, t, r, i) {
            let o = h(e, n.__wbindgen_malloc, n.__wbindgen_realloc),
              s = d,
              a = h(t, n.__wbindgen_malloc, n.__wbindgen_realloc),
              c = d,
              l = h(r, n.__wbindgen_malloc, n.__wbindgen_realloc),
              u = d,
              f = n.rewriter_rewrite_js(this.__wbg_ptr, o, s, a, c, l, u, i)
            if (f[2]) throw p(f[1])
            return p(f[0])
          }
          rewrite_js_bytes(e, t, r, i) {
            let o = (function (e, t) {
                ...
                return c().set(e, r / 1), (d = e.length), r
              })(e, n.__wbindgen_malloc),
              s = d,
              a = h(t, n.__wbindgen_malloc, n.__wbindgen_realloc),
              l = d,
              u = h(r, n.__wbindgen_malloc, n.__wbindgen_realloc),
              f = d,
        ...
        function y() {
          ...
          return (
            ...
            (e.wbg.__wbg_scramtag_3a255d78b157986d = function (e) {
              let t = h((0, i.t)(), n.__wbindgen_malloc, n.__wbindgen_realloc),
                r = d
              w().setInt32(e + 4, r, !0), w().setInt32(e + 0, t, !0)
            }),
            ...
            (e.wbg.__wbindgen_string_get = function (e, t) {
              ...
              var i =
                  null == r
                    ? 0
                    : h(r, n.__wbindgen_malloc, n.__wbindgen_realloc),
                o = d
              w().setInt32(e + 4, o, !0), w().setInt32(e + 0, i, !0)
            }),
        ...
        async function S(e) {
          ...
          ;("string" == typeof e ||
            ...
            (e = fetch(e))
          let { instance: r, module: i } = await _(await e, t)
          return v(r, i)
        }
      },
    },
  ...
  ;(r.d = (e, t) => {
    ...
    (() => {
      ...
      class o extends EventTarget {
        ...
        syncPool = {}
        synctoken = 0
        cookieStore = new n.h3.shared.CookieStore()
        serviceWorkers = []
        constructor() {
          super(), (this.client = new n.h3.shared.util.BareClient())
          let t = indexedDB.open("$scramjet", 1)
          ;(t.onsuccess = () => {
            let e = t.result
              ...
              .get("cookies")
            e.onsuccess = () => {
              e.result && this.cookieStore.load(e.result)
            }
          }),
            addEventListener("message", async ({ data: r }) => {
              if ("scramjet$type" in r) {
                if ("scramjet$token" in r) {
                  let e = this.syncPool[r.scramjet$token]
                  delete this.syncPool[r.scramjet$token], e(r)
                  return
                }
                if ("registerServiceWorker" === r.scramjet$type)
                  return void this.serviceWorkers.push(
                    new e.Y(r.port, r.origin),
                  )
                "cookie" === r.scramjet$type &&
                  (this.cookieStore.setCookies([r.cookie], new URL(r.url)),
                  t.result
                    .transaction("cookies", "readwrite")
                    .objectStore("cookies")
                    .put(JSON.parse(this.cookieStore.dump()), "cookies")),
                  "loadConfig" === r.scramjet$type && (this.config = r.config)
              }
            })
        }
        async dispatch(e, t) {
        ...
        async fetch({ request: e, clientId: r }) {
          this.config || (await this.loadConfig())
          let n = await self.clients.get(r)
          return t.pd.call(this, e, n)
        }
      }
